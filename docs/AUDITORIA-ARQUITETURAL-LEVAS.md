...
| L7 | `saveTournament` / `AppStore` e caminhos paralelos | **Em execução. Núcleo crítico publicado até 2.2.31:** resultado, geração/publicação de chave, rodada de Liga, reabertura de inscrições, dissolução de times, sorteio de vagas, enquete, grupos e promoção de fase passam pela mutação transacional. Categoria agora reaplica sobre o elenco fresco remoção, movimentação, atribuição, mesclagem e os dois modos de desfazer, cobertos por gate específico. **Restam writers administrativos** de perfil/categoria, inscrições/duplas, criação e manutenção de chave; o censo bruto encontra 66 chamadas a classificar, pois inclui caminhos já transacionais, compatibilidade e operações de desenvolvimento. | Migrar cada writer restante por intenção, com teste de corrida; não substituir por cópia/delta genérica que possa apagar alteração concorrente. |
