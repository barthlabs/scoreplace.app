...
| L10 | **etapa 1 concluída em 14/set (2.3.20)** | Vite gera o único artefato público e o Hosting o verifica; o restante da migração deixa de depender da raiz do repositório |
| L11 | **etapa 2 concluída em 14/set (lote 2.3.22)** | contratos tipados cobrem limites de rodadas, identidade/composição, lista de espera e classificação, sem converter o app global de uma vez |

**Decisão de release da L10, 14/set.** Esta é uma publicação exclusivamente web. O número
do shell e do Hosting avança para 2.3.20; os arquivos iOS/Android e qualquer sincronização do
Capacitor ficam intocados até a aprovação da versão já submetida ao Google. Não é exceção
...
### Recalibração — encerramento da etapa 5 (14/set/2026)
...
### Recalibração — encerramento da etapa 1 (14/set/2026)

A previsão original para a etapa 1 era **2–3 dias efetivos**. O escopo foi entregue sem
abrir dependência nativa: Vite monta `www/`, o Hosting só serve esse artefato e as travas
conferem entradas, hashes, versão e exclusão de fontes internas. O ciclo final mensurável de
validação/publicação levou **538 s**: 321 s de suíte+prerender, 85 s de upload e 25 s de
confirmação pública; as **822 suítes** passaram. A implementação anterior não tem relógio de
trabalho confiável consolidado, então não registro uma falsa duração humana a partir desses 538
s de máquina.

A evidência não revelou risco novo para a próxima etapa. A estimativa de **6–8 dias efetivos**
para a etapa 2 permanece: a infraestrutura permite domínios pequenos e verificáveis, mas
identidade tem 122 consumidores e exige migração compatível em cliente e motor. As estimativas
das demais etapas permanecem as do plano PDF até existir medição comparável. A revisão Claude
ocorre uma vez por etapa, dentro do fluxo de publicação; não haverá pré-revisão duplicada.
...
| L10 | source → artefato verificável | **Etapa 1 concluída em 14/set, publicada em 2.3.20.** O domínio de limites de rodada nasce em `src/domain/round-bounds.ts`; `scripts/build-domain.js` o compila para `js/domain/round-bounds.js` e falha se os dois divergirem. O Vite passou a orquestrar `build:www`, preservando a ordem dos scripts clássicos e emitindo manifesto; o pacote também registra hashes estáveis. O Hosting passou a ter `www/` como única raiz pública e o predeploy monta e confere esse artefato, incluindo versão, política remota e favicon. A publicação é deliberadamente web-only: não toca iOS/Android nem cria bundle Capacitor enquanto a aprovação Google estiver pendente. | A etapa 2 migra os próximos domínios puros e reduz globais antes de converter a entrada clássica em módulos. |
| L11 | TypeScript progressivo + Firebase compat → modular | **Etapa 2 concluída em 14/set no lote 2.3.22.** `round-bounds`, `participant-identity`, `waitlist` e `standings` compilam em TypeScript estrito e preservam adaptadores clássicos. Identidade e composição estrutural de dupla são carregadas antes dos consumidores do navegador e do sorteio; espera também alimenta a Function transacional de disponibilidade; classificação alimenta o motor de sorteio. O vendor verifica o JavaScript gerado byte a byte. A bateria final desta etapa tem 825 suítes e não toca os projetos nativos. | A etapa 3 pode retirar os primeiros adaptadores de compatibilidade e isolar as portas Firebase depois que os consumidores já chamarem contratos explícitos. |
...
| L16 | observabilidade e hardening | **P0–P3 entregues — o rótulo "Aberta" estava vencido.** O inventário separou o erro engolido DE PROPÓSITO do engolido CALADO, e os calados passaram a aparecer. Segue aberta a parte de métricas/alertas por risco. | Métricas, alertas e runbooks definidos por risco, sem registrar PII. |
...
2.1.28** e escreve direto em `notif_email_queue`. Fui conferir o bundle.

**MEDIDO agora, nos três embarcados do repositório**
(`android/app/src/main/assets/public`, `ios/App/App/public`, `www`):

| o que o documento diz | o que está lá |
|---|---|
| bundle embarcado em **2.1.28** | **2.2.84** — o registro está **56 versões** atrasado |
| o bloqueio é do **parque instalado** | a escrita direta **sobrevive na 2.2.84**: 1 por bundle, `js/firebase-db.js` → `collection('notif_email_queue').add(...)` |

⭐ **A conclusão continua de pé e a razão MUDA de dono.** Não é "o parque está velho e a gente
espera" — é **o nosso código, na versão mais nova, ainda escrever direto**. A primeira é uma
espera; a segunda é trabalho nosso, e está ao nosso alcance.

⛔ **Por isso a Rule segue fechada para fechar**: publicar uma versão nova não resolve nada
enquanto `queueNotifEmail` for uma escrita de cliente. O caminho é o já decidido pelo dono —
**capability específica de intenção**, nunca Function genérica que aceite destinatário e HTML
do cliente. Hoje o writer é **UM só** (`js/firebase-db.js:3291`), alcançado por um único
chamador (`_dispatchChannels`), o que torna a migração pequena e localizada.
⚠️ E a L14.P5 já pôs a dedup do lado do servidor, então a porta nova nasce com a reentrega
resolvida.

**Estado das lojas, medido na App Store Connect (não deduzido):**
- **iOS 2.2.81 está APROVADA mas NÃO PUBLICADA** — `PENDING_DEVELOPER_RELEASE`, com
  `release: MANUAL`. Quem está na loja hoje é a **2.2.8**. Aprovação não é publicação: falta
  apertar o botão de liberar.
- As demais na conta: 2.2.8, 2.2.0, 2.1.28 e 2.1.22, todas `READY_FOR_SALE`.
- ⚠️ Android **não foi medido aqui** (não há script de Play neste repositório); o registro diz
  Produção em 2.2.0 e a 2.2.84 em análise.
...
**L3.P3 — estatísticas e troféus passaram a ler o schema real (08/set/2026).**

Este reparo não restringe Rules, não escolhe autoridade e não reescreve salas existentes.
Ele fecha o defeito de leitura confirmado na L3.P1: tanto o cliente quanto
`backfillAllUserTrophies` buscavam `hostUid`/`guestUid` e comparavam `winner` na raiz,
campos que a medição encontrou em **0/17** documentos. O resultado eram os três contadores
casuais em zero para todos os 263 perfis materializados, apesar dos 15 jogos terminados.

O núcleo puro `casual-stats-core` agora é a fonte da qualificação, da união de identidades,
do limite diário e da vitória. Ele considera `playerUids`, `players[].uid` e
`participants[].uid`; a vitória exige que `players[].team` seja igual a
`result.winner`. Cliente e Function consultam o índice `playerUids array-contains`, com
`createdBy` apenas como complemento para sala legada, e deduplicam por id. Um jogador sem
time reconhecível pode contar como participante, mas nunca recebe vitória inferida.

O teste `casual-stats-canonicos` cobre a forma persistida, empate, bot, autojogo, duração
mínima e os dois caminhos de consulta; `check-casual-stats-core-fresh` obriga a cópia do
navegador a ser idêntica à da Function. A suíte completa da versão 2.2.27 passou com esses
gates. A limitação permanece explícita: uma sala antiga em que o uid exista **somente** em
`participants` e não em `playerUids` nem `createdBy` não é alcançável por uma consulta
segura por usuário; a L3.P1 mediu `playerUids` não-vazio em 17/17 documentos vigentes.
...
**L7.P1.27 — check-in comum da organização por intenção tipada (10/set/2026; publicado em 2.2.46).** O caminho rápido de presença deixou de encaminhar listas `sets`/`dels` para a porta
...
mapas separados, e prova autorização, lote único e limpeza. Com a autopresença fora das duas mutações locais, o censo cai de **82 para 80** writers; o gate
aceita somente essa direção.


**L7.P1.28 — família operacional de W.O. por intenção transacional (10/set/2026; publicada em
2.2.47, revisada e aprovada pelo Claude).** A declaração da organização, a presença que pode
preencher uma vaga, a escolha explícita de suplente que quebra categoria e a reversão de W.O.
não usam mais `AppStore.mutate` no navegador. Cada tela envia somente UID/nome e intenção
absoluta; `applyTournamentWO`, `setTournamentPresenceWithWOSubstitution`,
`resolveWOSubstitutionChoice` e `setTournamentWOAbsence` relêem o torneio fresco, exigem
organização por UID, validam alvo/pendência/opção no elenco canônico e gravam pelo mesmo
planejador de torneio dividido. O motor puro foi concentrado em `wo-core.js`, vendorizado para
a Function: não há segunda regra de W.O. no servidor. A reversão preserva a trava de partida já
jogada; a escolha excepcional aceita somente UID oferecido na pendência. Gates cobrem UID
homônimo, categoria, presença com TTL, reversão idempotente, cliente sem escrita, vendor e
catálogo; `npm test` fechou 694 suítes verdes e Claude aprovou o diff. O censo deve ser
...
**L7.P1.33 — vínculo de participante pelo servidor (10/set/2026; publicado na 2.2.49).** Pedido, aceite e recusa de vínculo não reescrevem `pendingMerges`, elenco ou chave no navegador. As Functions relêem o torneio, exigem a organização para pedir e o UID indicado para decidir; no aceite, a troca de identidade e a remoção da pendência compartilham uma transação. O censo cai de **66 para 63** writers iniciados pelo navegador.

**L7.P1.32 — placeholder ocupado só pelo servidor (10/set/2026; publicado na 2.2.49).** Arrastar um suplente sobre `Jogador NN` agora envia somente o UID e o nome da vaga para `occupyTournamentPlaceholder`. A Function relê a espera no documento fresco, exige a organização e troca chave, elenco, espera, membros e histórico dentro da mesma transação; a tela só redesenha depois do retorno confirmado. O gate impede `AppStore.mutate` e prova que o estado anterior é capturado antes da escrita. O censo cai de **68 para 66** writers iniciados pelo navegador.

**L7.P1.31 — fechamento de inscrição pelo servidor (10/set/2026; publicado na 2.2.49).** O relógio do dashboard e a varredura do boot não gravam mais `status: closed`, nem de forma otimista nem via transação local. Ambos enviam uma única intenção deduplicada por torneio para `closeExpiredEnrollment`; a Function relê prazo, organização e chave antes de decidir. Os gates exigem ausência de `commitTournamentTx`/`update` nos dois caminhos e o censo cai de **70 para 68** writers iniciados pelo navegador.

**L7.P1.30 — presença sem fotografia otimista (10/set/2026; publicado na 2.2.49).** O
...
**L8.P1 — ficha e limpeza de resíduos (11/set/2026).** A ficha já usa
`collectionGroup('results')` como caminho primário; as consultas locais de até 40 torneios só
abrem quando a consulta agrupada falha, e o gate exercita os dois ramos. O conferidor read-only
ganhou relatório estruturado de órfãos, incluindo placar, W.O., replay, proposta pendente e o
contador de `playerUids`. A medição global posterior encontrou **60 torneios, 282 jogos canônicos,
0 ausentes, 0 divergentes e 8 órfãos**; todos os oito pertenciam ao Confra e não continham dados
não deriváveis. `remover-orfaos-espelho-resultados.js` só aceita um `tid`, relê a fonte canônica,
recusa qualquer divergência ou conteúdo sensível, apaga somente os IDs comprovados e repete a
auditoria como pós-condição. A operação removeu os oito resíduos e a releitura confirmou
**214 jogos canônicos, 0 ausentes, 0 divergentes e 0 órfãos no Confra**.
...
## L9 — compatibilidades e código sem chamador (11/set/2026)

**L9.P0 — inventário com prova de uso antes de apagar.** A varredura alcançou os arquivos de
aplicação e as suítes, separando definição de chamada. Três aliases globais tinham somente a
própria definição e nenhum HTML, rota ou script os invocava: `_reopenSet`,
`_messageOrganizer` e `_onRankingManualChange`. Foram removidos. Os caminhos canônicos —
`_editSetsInline` e `_contactOrganizer` — continuam expostos e a suíte
`l9-aliases-mortos-removidos` impede que os apelidos retornem por cópia acidental.

As demais compatibilidades foram mantidas porque possuem motivo verificável: `_verMaisTag` é
usado pelos renders e pelas suítes extraídas da dashboard; `_onLigaInactivityChange` ainda é
chamado pela tela; `_propagateNameChange` é chamado ao salvar perfil, embora seja uma casca
server-only; e as funções de W.O. preservam a transição de escolhas antigas para o motor
transacional. Os aliases de catálogo de notificação também não entram nesta limpeza: são chaves
persistidas em documentos históricos, não código morto. Assim a L9 removeu apenas superfície
sem uso sem perder leitura de dados já publicados.

**L9.P1 — censo de formatos e nova prova de ausência (11/set/2026).** O censo read-only
`scripts/censo-formatos-legados.js` leu os **60** documentos de torneio sem escrever: **41** usam
fonte dividida e **19** são inteiros; **7** têm `combinedCategories`, e **nenhum** depende de
`genderCategories`/`skillCategories` para reconstruir `combinedCategories`. Isso não autoriza
apagar os eixos: os writers atuais ainda os gravam para regras semânticas como Misto Obrigatório.
Há **11** documentos com configuração legada de Liga, então esse fallback continua necessário.
O gate puro testa a classificação e proíbe verbos HTTP de mutação no censo.

No mesmo inventário, `addBotsFunction` era um writer de snapshot inteiro sem nenhum chamador no
produto, em contraste com `addTournamentPlaceholders`, que já é uma intenção de Cloud Function.
