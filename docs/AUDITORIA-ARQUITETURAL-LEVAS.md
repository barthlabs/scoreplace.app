...
| L11 | **etapa 2 em execução desde 14/set (lote 2.3.21)** | contratos tipados começam por limites de rodadas e identidade; migrar espera e classificação sem converter o app global de uma vez |
...
| L11 | TypeScript progressivo + Firebase compat → modular | **Etapa 2 em execução em 14/set.** `round-bounds` e `participant-identity` compilam em TypeScript estrito e preservam pontes clássicas compatíveis. `participant-identity` governa `participantUids` e a detecção estrutural de dupla, é carregado antes de `identity-core` tanto no shell quanto no motor de sorteio, e o vendor compara byte a byte o JavaScript gerado. O lote web 2.3.21 publica essa fronteira sem tocar os projetos nativos. | Extrair espera e classificação para contratos tipados; a troca de Firebase compat fica depois que os consumidores tiverem fronteiras explícitas. |
