...
| L10 e L11 | **em execução desde 14/set** | primeira fronteira tipada e build reproduzível concluídos; o Hosting passa a servir somente o artefato Vite verificado; migrar os próximos domínios sem converter o app global de uma vez |
...
| L10 | source → artefato verificável | **Iniciada em 14/set.** O domínio de limites de rodada nasce em `src/domain/round-bounds.ts`; `scripts/build-domain.js` o compila para `js/domain/round-bounds.js` e falha se os dois divergirem. O Vite passou a orquestrar `build:www`, preservando a ordem dos scripts clássicos e emitindo manifesto; o pacote também registra hashes estáveis. O Hosting passou a ter `www/` como única raiz pública e o predeploy monta e confere esse artefato, incluindo versão, política remota e favicon. A publicação 2.3.20 é deliberadamente web-only: não toca iOS/Android nem cria bundle Capacitor enquanto a aprovação Google estiver pendente. | Migrar os próximos domínios puros e reduzir os globais antes de converter a entrada clássica em módulos. |
| L11 | TypeScript progressivo + Firebase compat → modular | **Iniciada em 14/set.** `round-bounds` compila em TypeScript estrito e preserva uma ponte clássica compatível; a regra pode ser consumida em CommonJS no servidor sem DOM. | Extrair identidade, espera e classificação para contratos tipados; a troca de Firebase compat fica depois que os consumidores tiverem fronteiras explícitas. |
