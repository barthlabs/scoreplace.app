...
| L8 | representações múltiplas de match + custo Firestore | **Parcial, medição de consistência concluída em 08/set.** `matches` é fonte e `results` é projeção para jogos divididos. O conferidor read-only mediu 57 torneios, 9 com jogos canônicos e 282 jogos: **71 `results` ausentes**, todos no Confra, e **0 divergentes**. A causa era cobertura incremental: gatilhos e plano de escrita criam/atualizam somente o jogo alterado; ausências legadas não voltavam a entrar no fluxo. O reconciliador administrativo restrito a um `tid`, revisado e aprovado pelo Claude, criou os 71 espelhos sem tocar em `matches` nem em `results` existentes. Pós-condição medida no Confra: **214 jogos canônicos, 0 ausentes, 0 divergentes**. Há **8 `results` órfãos** preservados para classificação, nunca apagados pelo reparo. | Medir reads/writes por tela, classificar os 8 órfãos e preservar `replay`/autorizações antes de convergir o modelo. |
