...
| L7 | `saveTournament` / `AppStore` e caminhos paralelos | **Em execução. Núcleo crítico publicado até 2.2.46:** resultado, geração/publicação de chave, rodada de Liga, reabertura de inscrições, dissolução de times, sorteio de vagas, enquete, grupos, promoção de fase e presença comum passam por Functions. A próxima leva local migra a família de W.O. (declaração, presença que substitui, escolha excepcional e reversão) para a mesma fronteira. **Restam writers administrativos** de perfil/categoria, inscrições/duplas, criação e manutenção de chave. | Migrar cada writer restante por intenção, com teste de corrida; não substituir por cópia/delta genérica que possa apagar alteração concorrente. |
...
**L7.P1.28 — família operacional de W.O. por intenção transacional (10/set/2026; local,
revisada pelo Claude; ainda não publicada).** A declaração da organização, a presença que pode
