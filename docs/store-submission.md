...
- [x] **Push NATIVO** — ✅ FEITO E VALIDADO no emulador `sp_test` (v4.3.29-beta). Plugin `@capacitor-firebase/messaging` + caminho nativo gated em `js/notifications.js` (token FCM nativo salvo em `users/{uid}.fcmToken`, listeners foreground/tap). Push de teste via FCM v1 API confirmado ponta a ponta: **background** → notificação na bandeja do SO; **foreground** → toast in-app. Sem conta paga (google-services.json + POST_NOTIFICATIONS já existiam). ⚠️ **Caveat de PROD (código PRONTO, falta só o deploy)**: o CF `sendPushNotification` mandava **data-only** (contrato anti-duplicata da web) — data-only NÃO gera tray notification confiável em background/killed no nativo. **Já preparado na branch** (commit `3c75e257`): o CF agora adiciona `notification`+`android` **só pros tokens `fcmTokenPlatform` começando 'native-'** (web grava 'web'/ausente → segue data-only, zero risco de duplicata). **NÃO deployado** — é PROD (functions-autodraw). Deploy pelo dono: `cd functions-autodraw && firebase deploy --only functions:sendPushNotification --project scoreplace-app` (alvo específico — footgun de deletar o outro codebase).
