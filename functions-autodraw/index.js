...
exports.applyEnrollmentAssignments = onCall(async (request) => {
  ...
  if(clean.some(x=>(!x.uid&&!x.name)||(x.gender===undefined&&x.category===undefined))) throw new HttpsError('invalid-argument','Alvo ou alteração inválida.');
  const ref=db.collection('tournaments').doc(tId),agoraIso=new Date().toISOString();
  return db.runTransaction(async tx=>{
    const t=await _leTorneio(tx,ref,tId); if(!t) throw new HttpsError('not-found','Torneio não encontrado.');
    ...
    for(const e of clean){const pools=e.waitlist?[t.waitlist,t.standbyParticipants,t.monarchWaitlist]:[t.participants]; let target=null; for(const pool of pools){target=find(pool,e);if(target)break;} if(!target) continue;
       ...
       * nunca gênero de pessoa. */
      if(e.gender!==undefined){if(e.pairMember){const _c=e.pairMember+'Gender',_f=e.pairMember+'GenderSource';if(e.gender){target[_c]=e.gender;target[_f]='organizador';}else{delete target[_c];delete target[_f];}}else if(e.gender){target.gender=e.gender;target.genderSource='organizador';}else{delete target.gender;delete target.genderSource;} changed++;}
...
// ─── Reabertura para completar vagas: intenção server-side ──────────────────
exports.reopenEnrollmentForTarget = onCall(async (request) => {
  const uid=request.auth&&request.auth.uid, data=request.data||{}, tId=String(data.tournamentId||'').trim();
  const target=Math.trunc(Number(data.target)||0), autoClose=data.autoClose===true;
  if(!uid) throw new HttpsError('unauthenticated','Entre na sua conta.');
