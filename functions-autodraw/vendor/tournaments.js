...
window._cancelPairRequest = function(tId, reqId) {
    ...
    window.FirestoreDB._callFn('cancelTournamentPairRequest', { tournamentId: String(tId), requestId: String(reqId) }).then(function(result) {
      if (result && result.tournament && typeof window._applyCFTournament === 'function') window._applyCFTournament(tId, result.tournament);
      if (typeof showNotification !== 'undefined') showNotification('Convite cancelado', '', 'info');
      if (typeof window._softRefreshView === 'function') window._softRefreshView();
    }).catch(function(err) {
      if (typeof showNotification !== 'undefined') showNotification('Não foi possível', window._pairErrorMsg((err && err.message) || ''), 'warning');
    });
};
...
window._autoPresenceFromVenue = function (t) {
  try {
    ...
    window.PresenceDB.loadMyActive(cu.uid).then(function (list) {
      ...
      if (!(window.FirestoreDB && typeof window.FirestoreDB.setTournamentPresence === 'function')) return;
      window.FirestoreDB.setTournamentPresence(t.id, cu.uid, 'present').then(function () {
        if (typeof showNotification === 'function') showNotification('✅ Presente', 'Você está no local do torneio — presença confirmada automaticamente.', 'success');
        if (typeof window._softRefreshView === 'function') window._softRefreshView();
      }).catch(function () {});
    });
  } catch (e) {}
...
function renderTournaments(container, tournamentId = null) {
    ...
    window._handleSortearClick = function (tId, isAberto, skipGates) {
        ...
        if (!skipGates && !_isLigaSort) {
            ...
            var _afterPresence = function () {
                ...
                if (typeof window._setTournamentEnrollmentStatus !== 'function') return;
                window._setTournamentEnrollmentStatus(tId, 'close', { forDraw: true }).then(function(saved) { if (saved && saved.changed !== false) _startDraw(); });
            };
    ...
    const renderTournamentCard = (t, isOrg) => {
        ...
        if (typeof window._requestExpiredLeagueSeasonClose === 'function') window._requestExpiredLeagueSeasonClose(t);
      ...
      (function() {
        ...
        function _buildDropzone() {
          ...
          return '<div class="sp-org-card sp-org-droptarget sp-org-dropzone" data-org-drop="1" title="Arraste um inscrito aqui para co-organizar" ' +
            ...
            'style="box-sizing:border-box;position:relative;align-items:center;justify-content:center;padding:8px 12px;background:rgba(251,191,36,0.05);border:2px dashed rgba(251,191,36,0.55);border-radius:10px;flex:1 1 13.5rem;max-width:100%;height:58px;">' +
...
window._partnerPickerRender = function(tId, q, enrolled, friends, searchResults) {
  ...
  function _item(p, badge, color) {
    var avatar = p.photo
      ? '<img src="' + window._safeHtml(p.photo) + '" style="width:2rem;height:2rem;border-radius:50%;object-fit:cover;flex-shrink:0;" onerror="this.style.display=\'none\'">'
