...
function _meName() { var c = _ligaContext(), u = (c && c.actor) || (window.AppStore && window.AppStore.currentUser); return u ? (u.displayName || u.name || '') : ''; }
function _meUid() { var c = _ligaContext(), u = (c && c.actor) || (window.AppStore && window.AppStore.currentUser); return u ? (u.uid || '') : ''; }
...
window._ligaAcceptSub = function (tId, inviteId) {
  // Fora do contexto canônico do servidor, a tela só envia a intenção.
  if (!_ligaContext() && window.FirestoreDB && typeof window.FirestoreDB._callFn === 'function') {
    window.FirestoreDB._callFn('acceptLigaSubstitutionInvite', { tournamentId: String(tId), inviteId: String(inviteId) }).then(function(result) {
      if (result && result.tournament && typeof window._applyCFTournament === 'function') window._applyCFTournament(tId, result.tournament);
      if (window.showNotification) window.showNotification('Você está jogando!', 'Entrou como substituto. Boa partida!', 'success');
      _rerender(tId);
    }).catch(function(err) { if (window.showNotification) window.showNotification('Convite', (err && err.message) || 'Não foi possível aceitar o convite.', 'error'); });
    return;
  }
...
window._ligaDeclineSub = function (tId, inviteId) {
  ...
  window.FirestoreDB._callFn('declineLigaSubstitutionInvite', { tournamentId: String(tId), inviteId: String(inviteId) }).then(function(result) {
    if (result && result.tournament && typeof window._applyCFTournament === 'function') window._applyCFTournament(tId, result.tournament);
    ...
    _rerender(tId);
  }).catch(function(err) { if (window.showNotification) window.showNotification('Convite', (err && err.message) || 'Não foi possível recusar o convite.', 'error'); });
};
...
window._ligaCancelInvite = function (tId, roundIndex, groupName) {
  ...
  if (!(window.FirestoreDB && typeof window.FirestoreDB._callFn === 'function')) return;
  window.FirestoreDB._callFn('cancelLigaSubstitutionInvites', { tournamentId: String(tId), roundIndex: roundIndex, groupName: String(groupName) }).then(function(result) {
    if (result && result.tournament && typeof window._applyCFTournament === 'function') window._applyCFTournament(tId, result.tournament);
    _rerender(tId); window._ligaPickFill(tId, roundIndex, groupName, _absent);
  }).catch(function(err) { if (window.showNotification) window.showNotification('Convite', (err && err.message) || 'Não foi possível cancelar o convite.', 'error'); });
};
