...
must(/\.bracket-sticky-scroll-wrapper\s*\{[\s\S]{0,900}container-type\s*:\s*inline-size/.test(css) &&
  ...
  /@container\s*\(min-width\s*:\s*600px\)[\s\S]{0,600}width\s*:\s*calc\(\(100% - 1rem\) \/ 2\)\s*!important/.test(css) &&
  /@container\s*\(min-width\s*:\s*880px\)[\s\S]{0,600}width\s*:\s*calc\(\(100% - 2rem\) \/ 3\)\s*!important/.test(css),
...
(async () => {
  ...
  await page.setContent('<style>' + css + '</style>' + track('phone', 500, 2, true) + track('medium', 650, 3, true) + track('wide', 920, 4, false));
  ...
  must(r.medium.slice(0, 2).every((x) => x.col >= 316 && x.col <= 318 && x.card >= x.col - 1),
    'em painel médio duas rodadas dividem a largura útil (' + r.medium[0].col.toFixed(0) + 'px cada)');
  must(r.wide.slice(0, 3).every((x) => x.col >= 295 && x.col <= 298 && x.card >= x.col - 1),
