...
const vote=body('_castPollVote','// ── Check for active polls'),close=body('_closePollEarly','// ── Restore enrollments'),reopen=body('_reopenPoll','// ── Apply poll result'),apply=body('_applyPollResult','window._handleP2Option');
[['voto',vote],['fechamento',close],['reabertura',reopen],['aplicação',apply]].forEach(([n,b])=>{ok(/AppStore\.mutate\(/.test(b),n+' usa mutação fresca');ok(!/saveTournament\(|AppStore\.sync\(/.test(b),n+' não grava snapshot inteiro')});
