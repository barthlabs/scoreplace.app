...
const ROOT = path.join(__dirname, '..'), PORT = 8105, PROJECT = 'demo-scoreplace';
const DRIVER = `
...
(async()=>{const org='uid_org';await req('PATCH','tournaments/t1','owner',{fields:{creatorUid:S(org),name:S('Torneio')}});const out={};
out.directDefinition=await req('PATCH','tournaments/t1?updateMask.fieldPaths=categoryDefinitions',org,{fields:{categoryDefinitions:{arrayValue:{values:[]}}}});
out.directRigor=await req('PATCH','tournaments/t1?updateMask.fieldPaths=enrollmentRigor',org,{fields:{enrollmentRigor:S('official')}});
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sp-catdef-')), config = path.join(dir, 'firebase.json'), driver = path.join(dir, 'driver.js');
...
const match = /__JSON__(\{.*\})/.exec(output); if (!match) throw new Error('driver sem resultado: '+output.slice(-800)); const out = JSON.parse(match[1]); let fail = 0;
function ok(value, message) { console.log((value ? '✓ ' : '✗ ') + message); if (!value) fail++; }
ok(out.directDefinition === 403, 'Rules negam categoryDefinitions direto, inclusive ao organizador');
ok(out.directRigor === 403, 'Rules negam enrollmentRigor direto, inclusive ao organizador');
ok(out.ordinaryAdminEdit === 200, 'edição administrativa não equivalente continua permitida');
