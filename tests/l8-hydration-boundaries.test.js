...
(async()=>{
 ...
 const text=fs.readFileSync(path.join(root,'js/views/bracket.js'),'utf8');const start=text.indexOf('  if (t && !t._resultsHydrated');const end=text.indexOf('\n  /* ⛔ PORTÃO',start);const code=text.slice(start,end);assert(start>=0&&end>start);
 let attempts=0,refresh=0;const t={id:'abc'};const ctx={t,window:{location:{hash:'#bracket/abcd'},AppStore:{hydrateMatchResults(){attempts++;if(attempts===1)throw new Error('offline');return Promise.resolve(true);}},_softRefreshView(){refresh++;}}};
 ...
 vm.runInNewContext(code,ctx);await drain();assert.equal(t._resultsHydrated,true);assert.equal(refresh,0,'prefixo de outro torneio não é a mesma rota');
 ...
 console.log('✓ hidratação: Firestore indisponível, vazio confirmado, conta concorrente, retry da chave e rota exata');
