...
ok(/isPending = outcome === 'pending'/.test(auto) && /isApproval = outcome === 'applied'/.test(auto), 'CF distingue proposta pendente de confirmação final');
ok(/PENDING_SCORE_APPROVAL_MS/.test(auto) && /releasePendingScoreApprovalNotifications/.test(auto), 'proposta só é liberada após uma hora pelo agendador');
ok(/superseded/.test(auto) && /_freshConsolidatedScoreNotification/.test(auto), 'proposta aprovada ou corrigida antes do prazo é descartada');
ok(/if \(!isPending && !isApproval\) return null;/.test(auto), 'lançamento direto e correção não viram aviso de placar');
