...
const hints=fs.readFileSync('js/hints.js','utf8');
assert(!bracket.includes('_advanceToElimination'));
assert(!hints.includes('_advanceToElimination'));
const phase=fs.readFileSync('js/views/phases-engine.js','utf8');
assert(phase.includes('advanceMultiPhase'));
console.log('✓ avanço legado sem chamador não conserva writer, botão ou dica no navegador');
