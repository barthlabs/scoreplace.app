...
const _profileLiveAlerts = require("./profile-live-alerts-core");
...
/* ⛔ O desenho do e-mail mora em `digest-core.js` (módulo PURO) — este arquivo não é
 * `require`-ável em teste, e regra que dói errar não pode viver aqui. */
const _digest = require("./digest-core.js");
const _digestEscape = _digest._digestEscape;
const _digestLevelMeta = _digest._digestLevelMeta;
const _digestPalette = _digest._digestPalette;
...
exports.updateOwnPreferredLocations = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid = request.auth && request.auth.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Login obrigatório");
    let locations;
    try { locations = _profileLocations.normalizePreferredLocations(request.data && request.data.locations); }
    catch (error) { throw new HttpsError("invalid-argument", error.message); }
    const db = admin.firestore(); const ref = db.collection("users").doc(uid);
    await db.runTransaction(async (tx) => {
      if (!(await tx.get(ref)).exists) throw new HttpsError("failed-precondition", "Perfil inexistente");
      tx.update(ref, { preferredLocations: locations, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
    });
    return { ok: true, locations: locations };
  }
);

exports.updateOwnNotificationPreferences = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid = request.auth && request.auth.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Login obrigatório");
    let preferences;
    try { preferences = _profileNotifications.normalizeNotificationPreferences(request.data && request.data.preferences); }
    catch (error) { throw new HttpsError("invalid-argument", error.message); }
    const db = admin.firestore(); const ref = db.collection("users").doc(uid);
...
exports.updateOwnFriendRequestPreference = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid = request.auth && request.auth.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Login obrigatório");
    let accept;
    try { accept = _profileFriendRequests.normalizeAcceptFriendRequests(request.data); }
    catch (error) { throw new HttpsError("invalid-argument", error.message); }
    const db = admin.firestore(); const ref = db.collection("users").doc(uid);
...
exports.updateOwnLiveAlerts = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid=request.auth&&request.auth.uid; if(!uid) throw new HttpsError("unauthenticated","Login obrigatório");
    let who; try { who=_profileLiveAlerts.normalizeLiveAlerts(request.data); } catch(error) { throw new HttpsError("invalid-argument",error.message); }
    const db=admin.firestore(),ref=db.collection("users").doc(uid);
    await db.runTransaction(async(tx)=>{if(!(await tx.get(ref)).exists)throw new HttpsError("failed-precondition","Perfil inexistente");tx.update(ref,{liveAlertsWho:who,liveAlerts:who!=="none",updatedAt:admin.firestore.FieldValue.serverTimestamp()});});
  ...
  async (request) => {
    ...
    if (roomCode) {
      ...
      const room = rooms.docs.find((doc) => {
        const data = doc.data() || {};
        return data.status === "active" && (data.createdBy === uid ||
          (Array.isArray(data.playerUids) && data.playerUids.includes(uid)) ||
          (Array.isArray(data.players) && data.players.some((player) => player && player.uid === uid)));
      });
      if (!room) throw new HttpsError("permission-denied", "Você não participa desta partida ativa");
    }
    await db.runTransaction(async (tx) => {
      const profile = await tx.get(profileRef);
      if (!profile.exists) throw new HttpsError("failed-precondition", "Perfil inexistente");
...
// A última configuração casual é uma conveniência de abertura, mas pertence ao
// perfil autenticado. O uid vem exclusivamente do token da chamada.
exports.updateOwnCasualLast = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid = request.auth && request.auth.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Login obrigatório");
    let preference;
    try { preference = _casualLastPreferences.normalize(request.data && request.data.preference); }
    catch (error) { throw new HttpsError("invalid-argument", error.message); }
    const db = admin.firestore();
    const ref = db.collection("users").doc(uid);
    await db.runTransaction(async (tx) => {
      if (!(await tx.get(ref)).exists) throw new HttpsError("failed-precondition", "Perfil inexistente");
      tx.update(ref, { casualLast: preference });
    });
    return { ok: true, preference };
  }
);

exports.updateOwnCasualScoringPreferences = onCall(
  { region: "us-central1", memory: "256MiB", timeoutSeconds: 30, cors: APP_ORIGINS },
  async (request) => {
    const uid = request.auth && request.auth.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Login obrigatório");
    let preferences;
    try { preferences = _casualScoringPreferences.normalize(request.data && request.data.preferences); }
    catch (error) { throw new HttpsError("invalid-argument", error.message); }
    const db = admin.firestore();
    const ref = db.collection("users").doc(uid);
    await db.runTransaction(async (tx) => {
      if (!(await tx.get(ref)).exists) throw new HttpsError("failed-precondition", "Perfil inexistente");
      tx.update(ref, { casualPrefs: preferences });
    });
    return { ok: true, preferences };
  }
);
...
exports.setLigaAvailability = onCall(
  ...
  async (request) => {
    ...
    /* ⛔ ESTA FUNÇÃO NÃO DEIXAVA RASTRO NENHUM — e isso custou uma investigação inteira.
     * Relato (11/set/2026): uma inscrita que levou W.O. não conseguia voltar para a lista de
     * espera. Sem um log aqui não havia como separar "ela não tocou", "o toque não chegou" e
     * "a função recusou" — e a ausência de linhas no Cloud Logging não prova nada (o mesmo
     * filtro devolve zero para `aplicarNoTorneio`, que roda o dia inteiro). Só uid e id de
     * torneio: nada de nome nem e-mail. [[L16.P0]] */
    console.log("[setLigaAvailability] pedido uid=" + callerUid + " tid=" + tournamentId + " isActive=" + (!!data.isActive));
    return db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      if (!snap.exists) throw new HttpsError("not-found", "torneio não existe");
      const t = await _splitParts.hidratar(tx, ref, snap.data() || {});
      ...
      try { update = _ligaAvailability.applyLigaAvailability(t, callerUid, data.isActive, _ligaDrawWindow); }
        ...
        console.warn("[setLigaAvailability] recusado uid=" + callerUid + " tid=" + tournamentId +
                     " motivo=" + (err && err.message ? err.message : "(sem mensagem)"));
        throw new HttpsError("failed-precondition", err && err.message ? err.message : "não foi possível alterar disponibilidade");
      }
      _splitParts.gravar(tx, ref, before, Object.assign({}, update, { updatedAt: new Date().toISOString() }));
      // Onde a pessoa FICOU. 'espera' é leitura conservadora: quem não está mais no elenco caiu
      // na fila ou saiu por outro motivo — o log diz o observado, não o suposto.
      const _naLista = (arr) => Array.isArray(arr) && arr.some((p) =>
        [p && p.uid, p && p.p1Uid, p && p.p2Uid].filter(Boolean).map(String).includes(String(callerUid)));
      console.log("[setLigaAvailability] ok uid=" + callerUid + " tid=" + tournamentId +
                  " isActive=" + (!!data.isActive) + " destino=" + (_naLista(update.participants) ? "elenco" : "espera"));
