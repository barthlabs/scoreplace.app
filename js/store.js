window.SCOREPLACE_VERSION = '2.3.103';
...
window._spLoaderLogoHtml = function () {
  ...
  // script): mexeu aqui, mexa lá — `tests/carregando-geometria-canonica.test.js`
  // trava os dois lados.
  // v1.8.75 (dono): "dobrar o tamanho do logo" — 76×57 → 152×114.
  // ⭐ 19/ago/2026 (dono): a exceção da v1.8.75 que deixava o SPLASH maior
  // (208×156, wordmark 1.8rem) MORREU — _"tem que ser canônico sempre do mesmo
  // tamanho nas mesmas posições todos os elementos"_. As telas aparecem em
  // SEQUÊNCIA na abertura e o logo mudando de tamanho entre elas era lido como
  // troca de tela. Agora o splash usa EXATAMENTE estas medidas: 152×114 +
...
// v2.3.30: APENAS para a saudação da hero box e o nome no link de perfil da
...
window.AppStore = {
  ...
  async saveUserProfileToFirestore() {
    ...
    try {
      ...
      var _contactPrivacy = {};
      ['omitEmail', 'omitPhone'].forEach(function(k) { if (user[k] !== undefined) _contactPrivacy[k] = user[k]; });
      if (Object.keys(_contactPrivacy).length) await window.FirestoreDB.saveContactPrivacy(_contactPrivacy);
