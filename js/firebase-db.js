...
window.FirestoreDB = {
  ...
  async saveLiveAlerts(who) {
    return this._callFn('updateOwnLiveAlerts', { who: who });
  },
  ...
  async saveLiveScorePreferences(preferences) {
    return this._callFn('updateOwnLiveScorePreferences', { preferences: preferences || {} });
  },

  async saveCasualLast(preference) {
  ...
  async savePushToken(uid, token, platform) {
    ...
    return await this._callFn('updateOwnPushToken', { token: token, platform: platform });
  },

  async unlinkOwnLinkedPhone(uid, phone) {
    var current = window.firebase && window.firebase.auth && window.firebase.auth().currentUser;
    if (!uid || !current || String(uid) !== String(current.uid)) throw new Error('unlinkOwnLinkedPhone exige o próprio uid autenticado');
