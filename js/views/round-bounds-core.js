...
(function () {
  ...
  /* A tela ainda expõe estas portas globais, mas as regras pertencem ao domínio
   * TypeScript carregado imediatamente antes. Falhar alto aqui é intencional: carregar
   * meia aplicação com a regra de prazo ausente é pior que impedir o boot. */
  var dominio = window.ScoreplaceRoundBounds;
  if (!dominio) throw new Error('ScoreplaceRoundBounds ausente antes de round-bounds-core.js');
  var _ms = dominio.toMillis;
  var _iso = dominio.toIsoBrt;
  var _ddmm = dominio.toDayMonthBrt;
  var _dias = dominio.formatDays;

  window._rbMs = _ms;
  window._rbIso = _iso;
  window._rbDDMM = _ddmm;
  window._rbDias = _dias;
  window._rbNormaliza = dominio.normalise;
  window._limitesDasRodadas = dominio.limitsOf;
  window._rbIguais = dominio.equalBounds;
  window._rbMove = dominio.moveBound;
  ...
  window._rbFaixas = dominio.labelLanes;
