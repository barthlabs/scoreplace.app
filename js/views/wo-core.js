...
// Aplica uma escolha JÁ autorizada sobre o torneio recebido. Esta é a forma
// transaction-safe usada pela Function; o wrapper por id abaixo fica apenas para
// compatibilidade de chamadas antigas da tela.
window._resolveWoSubChoiceToTournament = function (t, absentUid, subUid) {
  if (!t) return { ok: false, reason: 'no-tournament' };
  const choice = (Array.isArray(t.woSubChoices) ? t.woSubChoices : []).find((x) => x && x.absentUid === absentUid && !x.resolved);
  if (!choice) return { ok: false, reason: 'choice-not-pending' };

  if (!subUid) {
    choice.resolved = true;
    const absName = choice.absentName || (typeof window._memberNameByUid === 'function' ? window._memberNameByUid(t, absentUid) : '') || absentUid;
    return (typeof window._applyWO === 'function')
      ? window._applyWO(t, { absentName: absName, absentUids: [absentUid], scope: 'match', noSubBehavior: 'escalate', woScope: t.woScope || 'individual', _forceNoSub: true })
      : { ok: false, reason: 'wo-core-unavailable' };
  }

  // Aceite explícito: só uma opção que a pendência registrou pode quebrar a
  // categoria. Nunca se aceita um UID arbitrário enviado pela tela.
  const allowed = (choice.options || []).some((o) => o && String(o.uid || '') === String(subUid));
  if (!allowed) return { ok: false, reason: 'sub-not-offered' };
...
window._woResolveSubChoice = function (tId, absentUid, subUid) {
  ...
  return window._resolveWoSubChoiceToTournament(t, absentUid, subUid);
};

// Reverte ou declara W.O. no torneio recebido, sem fetch/save/DOM. Foi extraída
// da view para que a Function execute a mesma reversão da interface. `wantAbsent`
// é uma intenção absoluta para sobreviver a retries da transação.
