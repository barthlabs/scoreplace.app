...
(function () {
  ...
  window._schCardChip = function (t, m) {
    try {
      ...
      // ⭐ COM PROPOSTA NA MESA o botão MUDA (pedido do dono, 03/set): recebe o brilho PADRÃO que
      // varre o botão da esquerda pra direita (`sp-shine-dica`, finito — regra 1.9.90) e diz o que
      // há pra ver — "Veja data proposta", em fonte menor — em vez de convidar a propor de novo.
  ...
  window._schGroupChip = function (t, groupMatches) {
    try {
      ...
      return '<button type="button" class="btn btn-micro btn-shine hover-lift' + (_temPropostaG ? ' sp-shine-dica' : '') + '" onclick="' + open + '" style="background:#3b82f6;color:#fff;font-size:' + (_temPropostaG ? '0.62rem' : '0.72rem') + ';font-weight:800;padding:4px 9px;line-height:1.05;text-align:left;' +
        (_temPropostaG ? '' : '') + '">' +
