...
window._executeRemoval = function(tId, mode, method) {
    ...
    // A CONTA do resto é núcleo PURO e vive em draw-decisions.js (_applyRemainderRemoval)
    // — a MESMA função que a CF `drawRound` roda sobre o doc fresco quando o pacote traz
    // `remainder`. Aqui era inline: o servidor não conseguia chamá-la (esta função toca
    // document/save/notificação) e teria que reimplementar a conta = 2ª versão do código.
    // Ver docs/sorteio-ciclo-decisoes.md.
    var _res = window._applyRemainderRemoval(t, mode, method);
...
// entrada é {uid} pelado, `_soloNameOf` devolve '' e NENHUM avulso era encontrado — o
// painel "N sem dupla" não movia ninguém e o sorteio seguia com eles dentro. É o que a CF
// lê sempre (doc cru). Ver [[project_uid_identity_canon_locked]].
window._listSoloEntries = function (t) {
    var parts = Array.isArray(t.participants) ? t.participants : [];
    return parts.filter(function (p) { return p != null && !window._entryTeamMembers(p); });
};
...
window._soloMoveOut = function (tt, toWaitlist) {
    ...
    // Mantém só as DUPLAS (estrutura). O `!n ||` antigo mantinha "quem não tem nome" — que
    // na forma real do doc é TODO MUNDO com conta → os avulsos ficavam no elenco.
    tt.participants = (Array.isArray(tt.participants) ? tt.participants : []).filter(function (p) {
        return !!window._entryTeamMembers(p);
    });
    if (toWaitlist) {
        if (!Array.isArray(tt.waitlist)) tt.waitlist = [];
        // dedup por UID (window._entryIdKey). Era por nome: homônimo colidia e o 2º sumia
        // da espera; e com a forma real do doc (só uid, sem nome) o `wn &&` fazia o dedup
        // nunca casar. Ver [[project_uid_identity_canon_locked]].
        solos.forEach(function (p) {
            var k = window._entryIdKey(p);
            if (!tt.waitlist.some(function (w) { return k && window._entryIdKey(w) === k; })) tt.waitlist.push(p);
        });
    }
    return solos.length;
};
...
window._confirmP2Resolution = function (tId, option) {
    ...
    const t = window._findTournamentById(tId);
    if (!t) return;
    // ── A APLICAÇÃO da escolha é núcleo PURO: window._applyP2Resolution (draw-decisions.js)
    // — a MESMA função que a CF `drawRound` roda sobre o doc fresco quando o pacote traz
    // `p2`. Este handler fica só com o que é UI: ler os radios, remover o painel, toast,
    // navegação. Os radios viram `opts` (o servidor não tem DOM pra lê-los).
    // Ver docs/sorteio-ciclo-decisoes.md §5.
    var _pickRadio = document.querySelector('input[name="standby-pick"]:checked');
    var _modeRadio = document.querySelector('input[name="standby-mode"]:checked');
