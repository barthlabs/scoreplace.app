    ...
    if (tournamentId && visible.length === 1) {
      (function() {
        ...
        if (_cu2 && !_viewerIsOrg) {
          ...
          // Rótulo em 2 linhas concordando com o gênero do organizador:
          //   masculino → "Falar com o" / "Organizador"
          //   feminino  → "Falar com a" / "Organizadora"
          //   sem gênero → "Falar com o(a)" / "Organizador(a)" (_orgRoleLabel já é a forma neutra)
          var _pgl = String(_primaryGender || '').toLowerCase().trim();
          var _orgArticle = (_pgl === 'feminino' || _pgl === 'f') ? 'a' : (_pgl === 'masculino' || _pgl === 'm') ? 'o' : 'o(a)';
          _contactBtnHtml = '<button class="btn btn-whatsapp btn-sm hover-lift" style="display:inline-flex;align-items:center;gap:8px;margin-top:10px;" onclick="event.stopPropagation();window._contactOrganizer(\'' + window._safeHtml(String(_t.id)) + '\')">' +
            _waIcon +
            '<span style="display:flex;flex-direction:column;line-height:1.12;text-align:left;white-space:nowrap;">' +
              '<span>Falar com ' + _orgArticle + '</span>' +
              '<span>' + _orgRoleLabel + '</span>' +
            '</span></button>';
        }
